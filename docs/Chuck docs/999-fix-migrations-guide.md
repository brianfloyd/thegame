...
# Run the fix script (script still in scripts/)
...
```sql
\i archive/reference-docs/fix-vitalis-drain-migrations.sql
```

**Note:** This SQL file has been archived to `archive/reference-docs/` as it's a one-time fix script, not a regular migration.

#### Using psql directly:
```powershell
psql your-connection-string -f archive/reference-docs/fix-vitalis-drain-migrations.sql
```

 ...
 * 
 * Can be run standalone: node scripts/migrate.js
 * Or imported and called: require('./scripts/migrate')()
 */
...
function runMigrations() {
  ...
  console.log(`[Migrations] Database path: ${DB_PATH}`);
  console.log(`[Migrations] Migrations directory: ${MIGRATIONS_DIR}`);
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      console.log('[Migrations] No migrations directory found. Creating it...');
  ...
  for (const filename of migrationFiles) {
    ...
    try {
      ...
      if (err.message.includes('duplicate column name')) {
        console.log(`[Migrations] Column already exists, marking migration as applied: ${filename}`);
        insertMigration.run(filename);
        appliedCount++;
      } else {
        console.error(`[Migrations] Error applying migration ${filename}:`, err.message);
        db.close();
        throw err;
      }
    }
  ...
  console.log(`[Migrations] Complete. (${appliedCount} new, ${migrationFiles.length - appliedCount} skipped)`);
...
// If run directly (npm run migrate), execute immediately
